 ...
 * @since 12/16/13
 */
object StaticController extends SockJsController {
  def infoOpts = CORSOptions("OPTIONS", "GET")

  private def randomNumber() = 2L << 30 + Random.nextInt

  def info(websocketEnabled: Boolean, cookieNeeded: Boolean) = Action {
    implicit request =>
      Ok(Json.obj(
...
<body>
  ...
  private val iframePage = new IframePage(play.api.Play.configuration.getString("mirari.sockjs.clientUrl").getOrElse("https://d1fxtkz8shb9d2.cloudfront.net/sockjs-0.3.min.js"))
